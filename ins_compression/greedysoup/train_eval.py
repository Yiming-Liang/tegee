...
class get_data():
    ...
    load data model
    '''
    def __init__(self,args,taskname):
        self.tasknames = taskname 
    ...
    def make(self):
        ...
        # 打印tasknames列表
        print(self.tasknames)

        # 遍历tasknames列表中的每一个任务名
        for taskname in self.tasknames:
            ...
            # 根据addins决定如何设置source和target列
            if self.addins:     #add instructions
                df['source'] = df['instructions'].astype(str) + df['input'].astype(str)
                df['target'] = df['output'].astype(str)
            else:
        ...
        # 分割数据为训练、验证和测试集
        trainval_dt, test_dt = train_test_split(combined_data, random_state=42, train_size=0.9)
        train_dt, val_dt = train_test_split(trainval_dt, random_state=42, train_size=0.9)

        # 如果训练集的长度超过了指定的样本数，则进行截断
        if len(train_dt) >= args.samples:
            train_dt = train_dt.iloc[:args.samples]
...
def main(args):
    ...
    for md in args.model_list: #initialize a model
        data_md.append(args.data_fp+md)
        model_paths.append(f"{args.model_path}{md[4:-5]}")
    ...
    delta_model = AutoDeltaModel.from_finetuned(model_path, backbone_model=model)
    ...
    model.parallelize()
    dt = get_data(args, data_md)    #get data
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_fp", default='/cpfs/29cd2992fe666f2a/shared/public/self-ins/default/', type=str,
    ...
    parser.add_argument("--ignore_pad_token_for_loss", default=True, type=bool,
                        help="")
    parser.add_argument("--config", default='/cpfs/29cd2992fe666f2a/user/zhangge/xw/trainlarge/lr_0.0003/lorabase.json', type=str)
    parser.add_argument("--output_dir", default="outputs/lora/t5-base/", type=str)
    parser.add_argument("--lora_save_dir", default="/cpfs/29cd2992fe666f2a/user/zhangge/xw/greedysoup/gpt3_soup/test_base_5/", type=str)
    parser.add_argument("--loraconfig", default={}, type=dict)
    parser.add_argument("--model_path", default="/cpfs/29cd2992fe666f2a/user/zhangge/xw/greedysoup/gpt3_soup/test_base_5/", type=str)
    parser.add_argument("--model_list", nargs = '+')
    parser.add_argument("--bank_config", default = "/cpfs/29cd2992fe666f2a/user/zhangge/xw/greedysoup/info_dict_gpt3_7.json", type=str)
    parser.add_argument("--bank_ids", default = 0, type=int)
    parser.add_argument("--samples", default = 5, type=int)
    args = parser.parse_args()
    result = main(args)
    import json
    with open(args.lora_save_dir +'/{}/'.format(args.bank_ids) +"collect_result.jsonl", 'a') as fout:
